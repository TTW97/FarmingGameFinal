...
function preload() {
  ...
  barrelseedimg = loadImage("images/BarrelSeed.png");

  trashscoreimg = loadImage("images/TrashScore.png");

  marketfoodimg = loadImage("images/MarketFood.png");

  canwaterimg = loadImage("images/CanWater.png");

  farmerimg = loadImage("images/Farmer.png");

  paragraphimg = loadImage("images/Paragraph.png");
...
function setup() {
  ...
  how = createSprite((width/2) + 200, height - 390);
  how.addImage(howimg);

  inventory = createSprite(width - 1520, height - 774);
  inventory.addImage(inventoryimg);
  inventory.scale = 0.3;
  inventory.visible = false;

  barrelseed = createSprite(width - 1180,height - 684);
  barrelseed.addImage(barrelseedimg);
  barrelseed.scale = 0.3;
  barrelseed.visible = false;

  trashscore = createSprite(width - 730, height - 684);
  trashscore.addImage(trashscoreimg);
  trashscore.scale = 0.3;
  trashscore.visible = false;

  marketfood = createSprite(width - 580, height - 84);
  marketfood.addImage(marketfoodimg);
  marketfood.scale = 0.3;
  marketfood.visible = false;

  canwater = createSprite(width - 1580,height - 349);
  canwater.addImage(canwaterimg);
  canwater.scale = 0.3;
  canwater.visible = false;

  farmer = createSprite(width - 1280, height - 484);
  farmer.addImage(farmerimg);
  farmer.scale = 0.3;
  farmer.visible = false;

  paragraph = createSprite(width/2, height/2 + 10);
  paragraph.addImage(paragraphimg);
...
function draw() {
  ...
  if (gameState === "play") {
    ...
    textSize(20);
    fill("black");
    text("Score: " + score, width - 1105, height - 759);
    text("Seconds Left: " + mainCounter, width - 1105 , height - 786);
  ...
  if (gameState === "stop") {
    ...
    fill("black");
    textSize(50);
    text("Your Score: " + score, width - 1180, height - 759);
    text("Trashed Plants: " + trashed_plants, width - 1180, height - 684);
    text("Harvested Plants: " + harvested, width - 1180, height - 609);
    textSize(20);
    text("Press Space to play Again", width/2 - 260, height/2 + 110);
    text("Press H to go back to Menu", width/2 - 260, height/2 + 60);
    play.visible = true;
    play.x = width/2 - 120;
    play.y = height/2 - 60;
    gameMusic.stop();
  ...
  if (gameState === "home") {
    ...
    fill("black");
    textSize(20);
    text("Press Space to Play", (width/2) - 105, height - 510);
    text("Press C to see Controls", width/2 - 400, height - 290);
    text("Press T to see How to play", width/2 + 50, height - 290);
    play.x = (width/2) - 25;
    play.y = height - 590;
  ...
  if (gameState === "control") {
    ...
    fill("black");
    textSize(20);
    text("Press H to go back to Menu", width/2, height/2 - 200);
  ...
  if (gameState === "how") {
    ...
    fill("black");
    textSize(28);
    text("Press H to go back to Menu", width/2, height/2 - 310);
    text("Press O to see objective", width/2, height/2 - 280);
    textSize(20);
...
function howtoplay() {
  ...
  if (keyWentDown("t")) {
    ...
    barrelseed.visible = true;
    trashscore.visible = true;
    marketfood.visible = true;
    canwater.visible = true;
    farmer.visible = true;
    paragraph.visible = false;
